...
export default class Server extends EventEmitter {
  ...
  protected async sendRequest(req: http.IncomingMessage, res: http.ServerResponse, protocol: string) {
    ...
    try {
      ...
      console.log(error)
      req.destroy(new ProxyError('Error while fetching request body', ErrorType.unknown, error as Error))
      return
    }
  ...
  protected async receiveResponse(request: Request, response: Response, serverResponse: http.IncomingMessage, proxyResponse: http.ServerResponse) {
    ...
    try {
      ...
      console.log(error)
      proxyResponse.destroy(new ProxyError('Error while fetching response body', ErrorType.inconsistency, error as Error))
      return
    }
